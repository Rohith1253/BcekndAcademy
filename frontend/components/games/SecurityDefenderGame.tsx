...
import { CheckCircle2, XCircle, ArrowRight, ShieldAlert, HelpCircle } from "lucide-react";

interface SecurityDefenderGameProps {
  scenarios: GameScenario[];
  onComplete: (answers: Record<string, any>) => void;
}

export default function SecurityDefenderGame({ scenarios, onComplete }: SecurityDefenderGameProps) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between text-xs font-medium text-slate-400 pb-3 border-b border-white/[0.08]">
        <span>Security Defender {currentIndex + 1} of {scenarios.length}</span>
        <div className="flex items-center gap-1 text-cyan-400 font-mono">
          <ShieldAlert className="w-3.5 h-3.5" />
          <span>OWASP & API Hardening</span>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-cyan-500/20 bg-slate-950/80 p-5 space-y-2">
        <span className="text-[10px] font-mono font-bold uppercase text-cyan-400 bg-cyan-500/10 px-2.5 py-0.5 rounded">
          API Security Vulnerability Scenario
        </span>
      ...
      {showFeedback && (
        <div className={`rounded-2xl border p-5 space-y-2 animate-fadeIn text-xs ${
          ...
          <div className="flex items-center gap-2 font-bold text-sm">
            ...
            <span>{isCorrect ? "Security Vulnerability Hardened!" : "Inadequate Defense Strategy"}</span>
          </div>
          ...
          <div className="pt-3">
            ...
            >
              <span>{isLast ? "View Final Results" : "Next Security Scenario"}</span>

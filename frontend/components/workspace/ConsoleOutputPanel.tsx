...
export default function ConsoleOutputPanel({
  ...
  const isTimeout = executionResult?.status === "timeout";

  return (
    <div className="flex flex-col h-full bg-[#050712] border-t border-slate-800 text-slate-100 overflow-hidden select-text">
      
      {/* Panel Tabs Header */}
      <div className="flex items-center justify-between px-4 py-1.5 bg-slate-900/90 border-b border-slate-800 shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={onClear}
            className="flex items-center gap-1 px-2 py-0.5 rounded text-[11px] text-slate-400 hover:text-white hover:bg-slate-800 transition cursor-pointer"
            title="Clear output"
          >
      ...
      <div className="flex-1 overflow-y-auto p-3 font-mono text-xs scrollbar-thin">
        ...
        {isRunning && (
          <div className="flex items-center gap-2 text-cyan-400 py-2">
            ...
            <span>Running in isolated Web Worker sandbox...</span>
          </div>
        )}
          ...
          <div className="space-y-1.5">
            {!isRunning && logs.length === 0 && !executionResult?.error && (
              <p className="text-slate-500 italic py-2">
                Click "Run Code" or press <kbd className="px-1.5 py-0.5 rounded bg-slate-900 border border-slate-800 text-slate-300 font-mono">Ctrl + Enter</kbd> to execute.
              </p>
            )}
            ...
            {logs.map((log, idx) => (
              <div
                key={idx}
                className={`flex items-start gap-2 py-0.5 leading-relaxed rounded px-1.5 ${
                  log.type === "error"
              ...
              >
                <span className="text-[10px] text-slate-600 select-none shrink-0 font-sans">{log.timestamp}</span>
                <span className="whitespace-pre-wrap flex-1">{log.message}</span>
              </div>
            ))}

            {isTimeout && (
              <div className="mt-2 p-3 rounded-xl border border-amber-500/40 bg-amber-500/10 text-amber-200 text-xs flex items-start gap-2">
                <AlertTriangle className="w-4 h-4 text-amber-400 shrink-0 mt-0.5" />
                <div>
                  <strong className="block font-bold">Execution Timeout:</strong>
                  <span>The code exceeded the 3000ms safety threshold. The Web Worker was terminated to protect browser responsiveness.</span>
                </div>
              </div>
            )}

            {executionResult?.error && !isTimeout && (
              <div className="mt-2 p-3 rounded-xl border border-rose-500/30 bg-rose-500/10 text-rose-300 text-xs flex items-start gap-2">
        ...
        {activeTab === "tests" && (
          <div className="space-y-2">
            {testResults.length === 0 ? (
              <p className="text-slate-500 italic py-2">
                No automated test results recorded yet. Click "Run Code" to evaluate your exercise solution.
              </p>
            ) : (
        ...
        {activeTab === "simulator" && (
          <div className="p-3 space-y-4 font-sans">
            <div className="flex items-center gap-2 text-xs text-slate-300">
              ...
              <span>Backend Request Pipeline Simulation</span>
            </div>
            ...
            <div className="grid grid-cols-5 gap-2 text-center text-xs">
              ...
              <div className="p-2.5 rounded-xl border border-slate-800 bg-slate-900/80">
                ...
                <span className="text-white font-bold text-[11px]">Auth & Header Guard</span>
              </div>
              <div className="p-2.5 rounded-xl border border-slate-800 bg-slate-900/80">
              ...
              <div className="p-2.5 rounded-xl border border-slate-800 bg-slate-900/80">
                ...
                <span className="text-white font-bold text-[11px]">Handler Logic</span>
              </div>
              <div className="p-2.5 rounded-xl border border-slate-800 bg-slate-900/80">
